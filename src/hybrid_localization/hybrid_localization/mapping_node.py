...
class MappingNode(Node):
    def __init__(self):
        ...
        self.faiss_index = faiss.IndexFlatIP(self.embedding_dim) # Inner-product search
        ...
        self.min_keyframe_dist = 1.0   # meters - minimum distance between keyframes
        self.min_keyframe_angle = 0.3  # radians - minimum rotation between keyframes
        self.max_step_dist = 50.0      # meters - reject jumps larger than this (SLAM divergence)
        self.last_keyframe_pose = None
        
        # Subscribers
        # Note: LIO-SAM publishes optimized odometry on /lio_sam/mapping/odometry
        self.odom_sub = message_filters.Subscriber(self, Odometry, '/lio_sam/mapping/odometry', qos_profile=qos_profile_sensor_data)
        self.image_sub = message_filters.Subscriber(self, CompressedImage, '/camera/color/image_raw/compressed', qos_profile=qos_profile_sensor_data)
        
        # Synchronizer (approximate time synchronization)
        self.ts = message_filters.ApproximateTimeSynchronizer([self.odom_sub, self.image_sub], queue_size=50, slop=0.1)
        self.ts.registerCallback(self.sync_callback)

        self.get_logger().info("Mapping pipeline ready. Waiting for data...")

    def _pose_distance(self, pose_a, pose_b):
        """Euclidean distance between two [x,y,z,qx,qy,qz,qw] pose arrays."""
        return float(np.linalg.norm(np.array(pose_a[:3]) - np.array(pose_b[:3])))
    ...
    def sync_callback(self, odom_msg, img_msg):
        ...
        if abs(pos.x) < 1e-6 and abs(pos.y) < 1e-6 and abs(pos.z) < 1e-6:
            if self.frame_count <= 10:
                self.get_logger().info(f"Skipping pre-init origin pose (frame {self.frame_count})")
                return
        ...
        if self.last_keyframe_pose is not None:
            ...
            if dist > self.max_step_dist:
                self.get_logger().warn(
                    f"Rejecting divergent pose: {dist:.1f}m jump (frame {self.frame_count})")
                return

            # Skip if too close to last keyframe
            if dist < self.min_keyframe_dist and angle < self.min_keyframe_angle:
                return
        ...
        with torch.no_grad():
            ...
            # Normalize for cosine similarity (which corresponds to inner product if normalized)
    ...
    def save_map(self):
        ...
        # Save FAISS index
        faiss.write_index(self.faiss_index, os.path.join(map_dir, "map_index.faiss"))
        
        # Save poses
        np.save(os.path.join(map_dir, "keyframe_poses.npy"), np.array(self.keyframe_poses))
        np.save(os.path.join(map_dir, "keyframe_ids.npy"), np.array(self.keyframe_ids))
        
        self.get_logger().info(f"Saved map with {self.keyframe_count} keyframes to {map_dir}")
...
def main(args=None):
    ...
    try:
        ...
        node.get_logger().info("Keyboard interrupt, shutting down and saving final map...")
    finally:
        node.save_map()
        node.destroy_node()
        rclpy.shutdown()
